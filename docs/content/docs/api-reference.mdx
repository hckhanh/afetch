---
title: API Reference
description: Complete API documentation for afetch with detailed examples and type signatures
---
...
afetch automatically throws an error for non-2xx status codes:
...
When using afetch in TypeScript, you get full autocomplete for:
...
afetch is designed to be performant:

- **Small bundle size**: Minimal dependencies (only fast-url and Standard Schema spec)
- **Efficient URL building**: Leverages fast-url's optimized implementation
...
afetch adds minimal overhead to your bundle:
...
afetch works in all modern JavaScript environments:
