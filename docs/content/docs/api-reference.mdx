...
what-the-fetch supports explicit HTTP method specification using the `@method` prefix notation in your API paths. This gives you full control over which HTTP method is used for each endpoint.

### Method Prefix Syntax

Use the `@method` prefix at the beginning of your path:

```typescript
const api = {
  '@get/resource': { /* schema */ },
  '@post/resource': { /* schema */ },
  '@put/resource/:id': { /* schema */ },
  '@patch/resource/:id': { /* schema */ },
  '@delete/resource/:id': { /* schema */ }
};
```
...
If you don't use a method prefix, what-the-fetch automatically determines the HTTP method:

```typescript
const api = {
  // Without prefix - method is inferred
  '/users/:id': {
    params: z.object({ id: z.number() }),
    response: z.object({ id: z.number(), name: z.string() })
  },
...
// This uses GET (no body)
const user = await apiFetch('/users/:id', { params: { id: 123 } });

// This uses POST (has body)
const newUser = await apiFetch('/users', {
  body: { name: 'John', email: 'john@example.com' }
});
```

**Automatic method rules:**
- If the request has a `body` → `POST`
- If the request has no `body` → `GET`
