...
Specify HTTP methods explicitly using the `@method` prefix:
...
// Explicitly use different HTTP methods
const user = await apiFetch('@get/users/:id', { params: { id: 123 } });
const created = await apiFetch('@post/users', { body: { name: 'John', email: 'john@example.com' } });
...
<Callout title="Automatic Method Detection" type="info">
  If you don't specify a method prefix, what-the-fetch will automatically:
  - Use `POST` for requests with a body
  - Use `GET` for requests without a body
</Callout>
...
<Callout title="Parameterized Paths Require Schema" type="warn">
