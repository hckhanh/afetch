...
description: Quick start guide to install and use afetch in your JavaScript or TypeScript project
---
...
Install afetch using your preferred package manager:

<Tabs items={['bun', 'npm', 'jsr']}>
  <Tab value="bun">
    ```bash
    bun add afetch
    ```
...
<Callout title="TypeScript Support" type="info">
  afetch is written in TypeScript and includes type definitions out of the
...
Let's build a complete type-safe API client using afetch:
...
afetch works with any Standard Schema implementation:
...
afetch validates both requests and responses automatically:
...
afetch provides clear error messages for both validation and HTTP errors:
...
afetch automatically throws on non-2xx status codes:
