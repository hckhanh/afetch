---
title: Introduction
description: Learn about what-the-fetch – a type-safe API client with schema validation using Standard Schema
---
...
<Callout title="Why what-the-fetch?" type="info">
  Building API clients manually is error-prone and lacks type safety. what-the-fetch
...
<Callout title="Type Safety & Validation" type="success">
  what-the-fetch handles type-safe URL construction, automatic validation, and full
...
<Tabs items={['With what-the-fetch', 'Without what-the-fetch']}>
  <Tab value="With what-the-fetch">
    ```typescript
    import { createFetch } from 'what-the-fetch';
    import { z } from 'zod';
    
    // Define API schema once with HTTP methods
    const api = {
      '@get/users/:id': {
        params: z.object({ id: z.number() }),
        query: z.object({ fields: z.string().optional() }),
  ...
  <Tab value="Without what-the-fetch">
    ```typescript
